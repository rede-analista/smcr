...
- Será aberta a página com todas as informações de ações desde que já tenha pelo menos um pino cadastrado. Nesta página tem um total de 23 posições para configurações das ações, o cadastro das ções pode ser realizada considerando a informação do "Pino Origem".<br>

  NOTA 1: Oberse que para cadastro de Ações existem 3 páginas no total e cada página 23 posições de ações conforme o pino que foi cadastrado anteriormente.<br>
  ...
  ![image](https://github.com/rede-analista/smcr/assets/66534023/a1043e41-5c32-4ade-862d-f6070bf4efe4)
...
- Parâmetro ENVIA RECEPTOR
  - Ativa o envio de informação de status do pino a outro módulo ESP32, esta informaçẽo pode ser NÃO=0 / SIM=1<br>
...
- São usados 6 arrays com as informações das ações aU16_Acao1 do tipo uint16_t, aU16_Acao2 do tipo uint16_t e aU16_Acao3 do tipo uint16_t, os 3 array tem a mesma função sendo que cada um gerencia as ações de cada página de configuração.<br>
  ...
  - aU16_Acao[10][23] = Array de 10 linhas e 23 colunas. (cada array)
    - aU16_Acao[Propriedade][Pino]
    - aU16_Acao[0][x] Índice 0 = Relacao (Pino Origem) 0=Nenhum
    - aU16_Acao[1][x] Índice 1 = Relacao (Pino Destino) 0=Nenhum
    - aU16_Acao[2][x] Índice 2 = Acao 0=Nenhuma / 1=Liga / 2=Liga Daley / 3=Pisca / 4=Pulso / 5=Pulso Delay
    - aU16_Acao[3][x] Índice 3 = Tempo Acao ON (LIGA DELAY=2 / PISCA=3 / PULSO=4 / PULSO DELAY ON=5)
    - aU16_Acao[4][x] Índice 4 = Tempo Acao OFF (PISCA=3 / PULSO DELAY ON=5)
    - aU16_Acao[5][x] Índice 5 = Envia ao Receptor 0=Nao / 1=Sim
    - aU16_Acao[6][x] Índice 6 = Envia ao Telegram 0=Nao / 1=Sim
    - aU16_Acao[7][x] Índice 7 = Envia ao MqTT 0=Nao / 1=Sim
    - aU16_Acao[8][x] Índice 8 = Notifica Assitente 0=Nao / 1=Sim
    - aU16_Acao[9][x] Índice 9 = Acionamento Nível Alto=1 / Baixo=0

  - aS8_Acao1[2][23] = Array de 2 linhas e 23 colunas. (cada array)
