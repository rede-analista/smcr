...
String f_upTime() {
  ...
  resultado += String(uptime::getDays());
  resultado += " Horas:";
    ...
    if (f_retornaModulo(SERVIDOR_WEB.arg(0)) < vU8_totPinos) {
      ...
      switch (SERVIDOR_WEB.arg(1).toInt()) {
        case 0: // Parametro acao = 0(Nenhuma)
          ...
          break;
        case 254: // Parametro acao = 254(Envia Status)
          aI16_ControlHS[0][f_retornaIndiceHS(SERVIDOR_WEB.arg(0))] = vI_cicloHandshake;
...
int f_retornaIndiceModulo(uint8_t modulo) {
  ...
  if (modulo > 0) {
    for (int x=0; x<vU8_totPinos; x++) {
      if (modulo == f_subtrai65k(aU16_InterMod[0][x])) {
        resultado = x;
        break;
      }
    }
...
//========================================
uint16_t f_subtrai65k(uint16_t valor) {
  if (valor > 65000) {
    return valor - 65000;
  } else {
    return valor;
...
void f_checkAcoesModulos() {
  ...
  if ((agora - check_acoes_mod_lasttime) > vU16_modulos_MTBS_Acoes) {
    for (uint8_t x=0; x<vU8_totPinos; x++) {
      if ((aU8_AcaoRede1[0][x] >0 ) && aU16_Pinos_Status[0][x] && !aU8_ControlMsgMod[0][x]) {
        if (f_regModHist(aU8_AcaoRede1[0][x],aU16_Acao1[2][x], aU16_Acao1[5][x], aU16_Pinos_Status[0][x])) {
          aU8_ControlMsgMod[0][x] = 1;
        }
      } else if ((aU8_AcaoRede1[0][x] >0 ) && !aU16_Pinos_Status[0][x] && aU8_ControlMsgMod[0][x]) {
        if (f_regModHist(aU8_AcaoRede1[0][x],aU16_Acao1[2][x], aU16_Acao1[5][x], aU16_Pinos_Status[0][x])) {
      ...
      if ((aU8_AcaoRede2[0][x] >0 ) && aU16_Pinos_Status[0][x] && !aU8_ControlMsgMod[1][x]) {
        if (f_regModHist(aU8_AcaoRede2[0][x],aU16_Acao2[2][x], aU16_Acao2[5][x], aU16_Pinos_Status[0][x])) {
          aU8_ControlMsgMod[1][x] = 1;
        }
      } else if ((aU8_AcaoRede2[0][x] >0 ) && !aU16_Pinos_Status[0][x] && aU8_ControlMsgMod[1][x]){
        if (f_regModHist(aU8_AcaoRede2[0][x],aU16_Acao2[2][x], aU16_Acao2[5][x], aU16_Pinos_Status[0][x])) {
      ...
      if ((aU8_AcaoRede3[0][x] >0 ) && aU16_Pinos_Status[0][x] && !aU8_ControlMsgMod[2][x]) {
        if (f_regModHist(aU8_AcaoRede3[0][x], aU16_Acao3[2][x], aU16_Acao3[5][x], aU16_Pinos_Status[0][x])) {
          aU8_ControlMsgMod[2][x] = 1;
        }
      } else if ((aU8_AcaoRede3[0][x] >0 ) && !aU16_Pinos_Status[0][x] && aU8_ControlMsgMod[2][x]){
        if (f_regModHist(aU8_AcaoRede3[0][x], aU16_Acao3[2][x], aU16_Acao3[5][x], aU16_Pinos_Status[0][x])) {
...
//========================================
bool f_regModHist(uint8_t idmodulo, uint8_t acao, uint8_t pino, uint8_t valor){
  aU8_ControlMsgModHist[0][vU8_crtl_ModHist] = idmodulo;
  aU8_ControlMsgModHist[1][vU8_crtl_ModHist] = acao;
  aU8_ControlMsgModHist[2][vU8_crtl_ModHist] = pino;
  aU8_ControlMsgModHist[3][vU8_crtl_ModHist] = valor;
  vU8_crtl_ModHist++;
...
void f_enviaAcoesModulos() {
  ...
  if ((agora - envia_acoes_mod_lasttime) > (vU16_modulos_MTBS_Acoes+500)) {
    for (uint8_t x=0; x<vU8_totPinos; x++) {
      if (aU8_ControlMsgModHist[0][x] > 0) {
        if ( f_enviaModulo(aU8_ControlMsgModHist[0][x], String(aU8_ControlMsgModHist[1][x]), String(aU8_ControlMsgModHist[2][x]), String(aU8_ControlMsgModHist[3][x])) == 200) {
          aU8_ControlMsgModHist[0][x] = 0;
          aU8_ControlMsgModHist[1][x] = 0;
          aU8_ControlMsgModHist[2][x] = 0;
          aU8_ControlMsgModHist[3][x] = 0;
        }
      }
    }
...
void f_checkHandShake() {
  ...
  if ((agora - check_mod_lasttime) > vU16_modulos_HandShake) {
    for (uint8_t y=0; y<vU8_totPinos; y++) {
      ...
      if ( aU16_InterMod[0][y] < 65000 && aU16_InterMod[1][y] > 0 ) {
        if (aI16_ControlHS[0][y] < 2 && aU16_InterMod[1][y] > 0) { 
          aI16_ControlHS[2][y] = f_enviaModulo(f_subtrai65k(aU16_InterMod[0][y]),"255", String(f_subtrai65k(aU16_InterMod[0][f_retornaIndiceHS(vS_nomeDispositivo)])), "0");
          if (aI16_ControlHS[2][y] == 200) {
            aI16_ControlHS[0][y] = vI_cicloHandshake;
            aI16_ControlHS[1][y] = 0;
            aI16_ControlHS[0][f_retornaModulo(vS_nomeDispositivo)] = vI_cicloHandshake;
            aI16_ControlHS[1][f_retornaModulo(vS_nomeDispositivo)] = 0;
          }
        }
      } else if ( aU16_InterMod[0][y] > 65000 && vB_envia_Historico && aI16_ControlHS[1][y] == 1 ) {
        aI16_ControlHS[2][y] = f_enviaModulo(f_subtrai65k(aU16_InterMod[0][y]),"254", String(f_subtrai65k(aU16_InterMod[0][f_retornaIndiceHS(vS_nomeDispositivo)])), "0");
        if (aI16_ControlHS[2][y] == 200) {
          aI16_ControlHS[0][y] = vI_cicloHandshake;
          aI16_ControlHS[1][y] = 0;
          aI16_ControlHS[0][f_retornaModulo(vS_nomeDispositivo)] = vI_cicloHandshake;
      ...
      aU16_Pinos_Status[0][f_retornaIndicePino(f_subtrai65k(aU16_InterMod[0][y]))] = aI16_ControlHS[1][y];
    }
...
void f_salvaFlash() {
  ...
  //CONFIG_FLASH.putUChar("bits_proc", vU8_bitsProc);
  //CONFIG_FLASH.putUChar("cicl_samp", vU8_ciclosAmostra);
...
void f_grupoAcoes(bool force) {
  if (force) {
    for (int clear=0; clear<vU8_totPinos ;clear++) {
      for (int pinagem=0; pinagem<vI8_ControlAcoesGroups; pinagem++) {
        aU16_ControlAcoesGroups[pinagem][clear] = 0;
      }
    }
  }
  int xyz = 1;
...
//========================================
uint16_t f_lePino(uint8_t tipo, uint8_t invert, uint8_t pino) {
...
*/
  ...
  for (uint8_t x=0; x<vU8_totPinos; x++){
    ...
    if (aU8_Pinos[1][x] > 0 && aU8_Pinos[1][x] < 254) {
