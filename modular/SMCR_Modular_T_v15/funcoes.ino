...
bool f_validaNomeReceptor(String modulo) {
  ...
  for (uint16_t x=0; x<vS_nomeReceptor.length(); x++) {
    if (vS_nomeReceptor[x] == 44) {
      Serial.println("Virgula: "+ cmod);
      if (vS_nomeReceptor.indexOf(cmod) >=0) {
        Serial.println("Virgula2: "+ cmod);
        if (modulo == vS_nomeReceptor.substring(vS_nomeReceptor.indexOf(cmod),modulo.length())) {
          Serial.println("Virgula3: "+ cmod);
          resultado = true;
          interrompe = true;
          break;
        } else {
        ...
        Serial.println("Zera: "+ cmod);
        cmod = "";
      }
    } else {
      cmod += vS_nomeReceptor[x];
    }
  }
  if (!interrompe) {
    Serial.println("Interrompe: "+ cmod);
    if (vS_nomeReceptor.indexOf(cmod) >=0) {
      Serial.println("Interrompe2: "+ cmod);
      Serial.println(vS_nomeReceptor.substring(vS_nomeReceptor.indexOf(cmod),modulo.length()));
      if (modulo == vS_nomeReceptor.substring(vS_nomeReceptor.indexOf(cmod),modulo.length())) {
        Serial.println("Interrompe3: "+ cmod);
        resultado = true;
      }
    }
  }
  Serial.println("Final: "+ cmod);
  return resultado;
}
