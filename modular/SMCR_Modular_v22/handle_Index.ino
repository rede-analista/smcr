...
void f_handle_Index() {
  ...
  html += f_horaHTML();
  html += "Uptime:" + String(esp_timer_get_time());
  ...
  uint8_t x;
  html += "("+String(vU8_totPinos)+" pinos disponíveis)";
  html += "<table border='1'>";
  while (linha >= 0) {
    html += "    <tr>";
    ...
    html += "        <td style='background-color: LightGrey'><center>Status</center></td>";
                    for (uint8_t b=colINICIO; b<colFIM; b++){
                      if (aU8_Pinos_Status[0][b] && aU8_Pinos[0][b] > 0) {
                        html += "        <td style='background-color:"+vS_corStatus1+"'><center>"+String(aU8_Pinos_Status[0][b])+"</center></td>";
                      } else if (!aU8_Pinos_Status[0][b] && aU8_Pinos[0][b] > 0) {
                        html += "        <td style='background-color:"+vS_corStatus0+"'><center>"+String(aU8_Pinos_Status[0][b])+"</center></td>";
                      } else {
                        html += "        <td><center>"+String(aU8_Pinos_Status[0][b])+"</center></td>";
                      }
                    }
    html += "<tr>";
  ...
  for (uint8_t x=0; x<vI8_aU16_InterModHA; x++) {
      ...
      for (uint8_t y=0; y<vU8_totPinos; y++) {
        if (aS_InterMod[0][y].length() >0) {
          if ( x == 1) {
            if (aU16_ControlHS[x][y] && aU16_InterMod[0][y] > 0) {
