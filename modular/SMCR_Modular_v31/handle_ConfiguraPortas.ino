...
void f_handle_ConfiguraPortas() {
  ...
  while (linha >= 0) {
    ...
    for (uint8_t xM = 0; xM<vI8_aU16_Pinos;xM++) {
      html += "<tr>";
      html += "<td style='background-color: LightGrey'><center>"+aS16_PinosMenu[0][xM]+"</center></td>";
      for (x=colINICIO; x<colFIM; x++){
          html += "<td><input type='text' name='"+aS16_PinosMenu[0][xM]+String(x)+"' maxlength='14' size='14' value='"+aU16_Pinos[xM][x]+"'></td>";
      }
      html += "</tr>";
    }
    html += "<tr>";
  ...
  if (SERVIDOR_WEB.arg("SUBMIT_SALVAR").length() > 0) {
    ...
    for (x=0; x<vU8_totPinos; x++){
      ...
      for (uint8_t xM = 0; xM<vI8_aU16_Pinos;xM++) {
        aU16_Pinos[xM][x] = SERVIDOR_WEB.arg((aS16_PinosMenu[0][xM]+String(x))).toInt();
        if (aU16_Pinos[0][x] > 0 ) {
          if (aU16_Pinos[1][x] == 1 || aU16_Pinos[1][x] == 192) {
            pinMode(aU16_Pinos[0][x],aU16_Pinos[2][x]);
          }
        }
        if (xM == 0 && aU16_Pinos[xM][x] > 0){
