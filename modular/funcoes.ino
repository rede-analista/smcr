//========================================
String f_formataIp(String ipj) {
  String resultado = "";
  if (ipj.length() == 12 ) {
    resultado = ipj.substring(0,3).toInt();
    resultado += ".";
    resultado += ipj.substring(3,6).toInt();
    resultado += ".";
    resultado += ipj.substring(6,9).toInt();
    resultado += ".";
    resultado += ipj.substring(9,12).toInt();
  } else {
    resultado = "Formato_de_IP_Errado";
  }
  return resultado;
}
...
String f_upTime() {
  ...
  String resultado = String(uptime::getDays());
  resultado += " Dias - ";
  resultado += String(uptime::getHours());
  resultado += " Horas - ";
  resultado += String(uptime::getMinutes());
  resultado += " Minutos - ";
  resultado += String(uptime::getSeconds());
  resultado += " Segundos";
  return resultado;
}
...
void f_importarDoBackup() {
  ...
  vU16_modulos_HandShake = doc["time_mod"];
  vU16_modulos_MTBS_Acoes = doc["time_mod_ac"];
 ...
 // Atribuir valores aos arrays
  ...
  for (int i = 0; i < vI8_aU16_Pinos; i++) {
    for (int j = 0; j < vU8_totPinos; j++) {
      aU16_Pinos[i][j] = doc["aU16_Pinos"][i][j];
    }
  }
...
String f_exportarParaBackup() {
  ...
  DynamicJsonDocument doc(JSON_OBJECT_SIZE(1) +
                          ...
                          vI8_aU16_Pinos * JSON_ARRAY_SIZE(vU8_totPinos) +
                          vI8_aS8_Pinos * JSON_ARRAY_SIZE(vU8_totPinos) +
                          vI8_aU16_Acao * JSON_ARRAY_SIZE(vU8_totPinos) +
                          vI8_aU8_AcaoRede * JSON_ARRAY_SIZE(vU8_totPinos) +
...
//                          vI8_aS8_Acao * JSON_ARRAY_SIZE(vU8_totPinos));                          
  ...
  root["time_mod"] = vU16_modulos_HandShake;
  root["ciclo_mod"] = vI_cicloHandshake;
  root["time_mod_ac"] = vU16_modulos_MTBS_Acoes;
  ...
  serializeArray(aU16_Pinos, "aU16_Pinos", vI8_aU16_Pinos, vU8_totPinos, docExport);
  ...
  serializeArray(aU8_AcaoRede1, "aU8_AcaoRede1", vI8_aU8_AcaoRede, vU8_totPinos, docExport);
//  serializeArray(aS8_Acao1, "aS8_Acao1", vI8_aS8_Acao, vU8_totPinos, docExport);
  ...
  serializeArray(aU8_AcaoRede2, "aU8_AcaoRede2", vI8_aU8_AcaoRede, vU8_totPinos, docExport);
//  serializeArray(aS8_Acao2, "aS8_Acao2", vI8_aS8_Acao, vU8_totPinos, docExport);
  ...
  serializeArray(aU8_AcaoRede3, "aU8_AcaoRede3", vI8_aU8_AcaoRede, vU8_totPinos, docExport);
...
//========================================
bool f_retornaHSself() {
  bool resultado = false;
  for (uint8_t x=0; x<vU8_totPinos; x++) {
    if (aS_InterMod[0][x] == vS_nomeDispositivo) {
      resultado = true;
    }
  }
  return resultado;
...
bool f_retornaStatusGrupoAcao(String pinos, uint8_t status) {
  ...
  for (int x=0; x<=pinos.length();x++) {
    if (pinos[x] == 44) {
      xstat += aU16_Pinos_Status[0][f_retornaIndicePino(xpin.toInt())];
      xpin = "";
    } else {
      xpin += pinos[x];
    }
...
//========================================
String f_retornaGrupoAcao(uint16_t pino) {
  String resultado = "";
  int cont = 0;
...
//========================================
uint16_t f_retornaIndicePino(uint16_t pino) {
...
void f_iniciaInterModulos() {
  ...
  bool aBBuffer[vI8_aB_InterMod][vU8_totPinos] = {};
  ...
  CONFIG_FLASH.getBytes("paB_InterMod", aBBuffer, CONFIG_FLASH.getBytesLength("paB_InterMod"));
  CONFIG_FLASH.end();
...
*/
  ...
  if (SERVIDOR_WEB.args() == 4) {
    ...
    ULTIMOS_GET_RECEBIDOS += f_DataHora();
    ULTIMOS_GET_RECEBIDOS += " -> ";
    ...
    if (f_retornaModulo(SERVIDOR_WEB.arg(0)) < vU8_totPinos) {
      Serial.println("Recebido dados da placa " + SERVIDOR_WEB.arg(0));
      switch (SERVIDOR_WEB.arg(1).toInt()) {
        case 0: // Parametro acao = 0(Nenhuma)
          aI16_ControlHS[0][f_retornaIndiceHS(SERVIDOR_WEB.arg(0))] = vI_cicloHandshake;
          aU16_Pinos_Status[0][f_retornaIndicePino(SERVIDOR_WEB.arg(2).toInt())] = SERVIDOR_WEB.arg(3).toInt();
          validacao = "OK_DADO_RECEBIDO";
          break;
        case 65534: // Parametro acao = 65534(status)
          aI16_ControlHS[0][f_retornaIndiceHS(SERVIDOR_WEB.arg(0))] = vI_cicloHandshake;
          aI16_ControlHS[1][f_retornaIndiceHS(SERVIDOR_WEB.arg(0))] = SERVIDOR_WEB.arg(3).toInt();
          aU16_Pinos_Status[0][f_retornaIndicePino(SERVIDOR_WEB.arg(2).toInt())] = SERVIDOR_WEB.arg(3).toInt();
        ...
        case 65535: // Parametro acao = 65535(sincronismo)
          aI16_ControlHS[0][f_retornaIndiceHS(SERVIDOR_WEB.arg(0))] = vI_cicloHandshake;
          aI16_ControlHS[1][f_retornaIndiceHS(SERVIDOR_WEB.arg(0))] = SERVIDOR_WEB.arg(3).toInt();
          aU16_Pinos_Status[0][f_retornaIndicePino(SERVIDOR_WEB.arg(2).toInt())] = aI16_ControlHS[1][f_retornaIndiceHS(SERVIDOR_WEB.arg(0))];
          validacao = "OK_DADO_RECEBIDO";
          break;
        default:
          aI16_ControlHS[0][f_retornaIndiceHS(SERVIDOR_WEB.arg(0))] = vI_cicloHandshake;
          aI16_ControlHS[1][f_retornaIndiceHS(SERVIDOR_WEB.arg(0))] = 0;
          aU16_Pinos_Status[0][f_retornaIndicePino(SERVIDOR_WEB.arg(2).toInt())] = SERVIDOR_WEB.arg(3).toInt();
          validacao = "OK_DADO_RECEBIDO";
          break;
      }
      ...
      Serial.println("Rejeitado dados da placa " + SERVIDOR_WEB.arg(0));
      Serial.println("Acao/Indice/Status " + SERVIDOR_WEB.arg(1) + " " + SERVIDOR_WEB.arg(2) + " " + SERVIDOR_WEB.arg(3));
...
//========================================
uint16_t f_retornaIndiceModulo(uint16_t modulo) {
  int resultado = 0;
  if (modulo > 0) {
...
//========================================
String f_retornaIpModulo(uint16_t modulo) {
  String resultado = "";
  if (modulo > 0) {
    for (int x=0; x<vU8_totPinos; x++) {
      if (modulo == aU16_InterMod[0][x]) {
        resultado = f_formataIp(aS_InterMod[1][x]);
        break;
      }
    }
  }
  ...
  if ((agora - check_acoes_mod_lasttime) > vU16_modulos_MTBS_Acoes) {
    for (uint8_t x=0; x<vU8_totPinos; x++) {
      if ((aU8_AcaoRede1[0][x] > 0 ) && (aU16_Pinos_Status[0][x] >= aU16_Pinos[4][x]) && (aU16_ControlMsgMod[0][x] < 1)) {
        if (f_regModHist(aU8_AcaoRede1[0][x],aU16_Acao1[2][x], aU16_Acao1[5][x], aU16_Pinos_Status[0][x])) {
        ...
        f_regModHist(aU8_AcaoRede1[0][x],aU16_Acao1[2][x], aU16_Acao1[5][x], aU16_Pinos_Status[0][x]);
      }


      if ((aU8_AcaoRede2[0][x] >0 ) && (aU16_Pinos_Status[0][x] >= aU16_Pinos[4][x]) && (aU16_ControlMsgMod[1][x] < 1)) {
        if (f_regModHist(aU8_AcaoRede2[0][x],aU16_Acao2[2][x], aU16_Acao2[5][x], aU16_Pinos_Status[0][x])) {
        ...
        f_regModHist(aU8_AcaoRede2[0][x],aU16_Acao2[2][x], aU16_Acao2[5][x], aU16_Pinos_Status[0][x]);
      }


      if ((aU8_AcaoRede3[0][x] >0 ) && (aU16_Pinos_Status[0][x] >= aU16_Pinos[4][x])&& (aU16_ControlMsgMod[2][x] < 1)) {
        if (f_regModHist(aU8_AcaoRede3[0][x], aU16_Acao3[2][x], aU16_Acao3[5][x], aU16_Pinos_Status[0][x])) {
        ...
        f_regModHist(aU8_AcaoRede3[0][x],aU16_Acao3[2][x], aU16_Acao3[5][x], aU16_Pinos_Status[0][x]);
      }
    }
...
//========================================
bool f_regModHist(uint16_t idmodulo, uint16_t acao, uint16_t pino, uint16_t valor){
...
//========================================
void f_enviaAcoesModulos() {
  uint64_t agora = millis();
  if ((agora - envia_acoes_mod_lasttime) > (vU16_modulos_MTBS_Acoes+500)) {
    for (uint8_t x=0; x<vU8_totPinos; x++) {
      if (aU16_ControlMsgModHist[0][x] > 0) {
        if ( f_enviaModulo(aU16_ControlMsgModHist[0][x], String(aU16_ControlMsgModHist[1][x]), String(aU16_ControlMsgModHist[2][x]), String(aU16_ControlMsgModHist[3][x])) == 200) {
  ...
  if ((agora - check_mod_lasttime) > vU16_modulos_HandShake) {
    for (uint8_t y=0; y<vU8_totPinos; y++) {
      ...
      if ( aB_InterMod[0][y] && aU16_InterMod[0][y] > 0 ) {
        aI16_ControlHS[2][y] = f_enviaModulo(aU16_InterMod[0][y],"65535", String(aU16_InterMod[0][f_retornaIndiceHS(vS_nomeDispositivo)]), "0");
        if (aI16_ControlHS[2][y] == 200) {
          aI16_ControlHS[0][y] = vI_cicloHandshake;
          aI16_ControlHS[1][y] = 0;
          aI16_ControlHS[3][f_retornaModulo(vS_nomeDispositivo)] = 0;
        }
      }
      if ( f_retornaIndicePino(aU16_InterMod[0][y]) > 0 && f_retornaIndicePino(aU16_InterMod[0][y]) < 65535 ) {
        aU16_Pinos_Status[0][f_retornaIndicePino(aU16_InterMod[0][y])] = aI16_ControlHS[1][y];
      }
    }
...
//========================================
int f_enviaModulo(uint16_t idmodulo, String acao, String pino, String valor){
  vI_httpResponseCode = 0;
  int resposta =  504;
  if (aI16_ControlHS[3][f_retornaIndiceModulo(idmodulo)] < vU8_tentativaConexoes) {
    if (WiFi.status() != WL_CONNECTED) {
      ...
      if (resposta == 200) {
        if (vS_payload == "OK_DADO_RECEBIDO") {
          ...
          aI16_ControlHS[0][f_retornaIndiceModulo(idmodulo)] = vI_cicloHandshake;
          aI16_ControlHS[1][f_retornaIndiceModulo(idmodulo)] = 0;
          aI16_ControlHS[2][f_retornaIndiceModulo(idmodulo)] = resposta;
          aI16_ControlHS[3][f_retornaIndiceModulo(idmodulo)] = 0;
        }
      } else {
        aI16_ControlHS[2][f_retornaIndiceModulo(idmodulo)] == resposta;
        if (aI16_ControlHS[3][f_retornaIndiceModulo(idmodulo)] < 32766) {
          aI16_ControlHS[3][f_retornaIndiceModulo(idmodulo)]++;
        } else {
          aI16_ControlHS[3][f_retornaIndiceModulo(idmodulo)] = 1;
        }
        
      }
      Serial.println("\nEnviado: "+GET_SERVIDOR);
      Serial.println("HTTP Response code: " + String(resposta));
      Serial.println(payload);
      Serial.print("Pausa no envio: ");
      Serial.println(aI16_ControlHS[3][f_retornaIndiceModulo(idmodulo)]);
    } 
  } else {
      aI16_ControlHS[0][f_retornaIndiceModulo(idmodulo)] = vI_cicloHandshake;
      aI16_ControlHS[1][f_retornaIndiceModulo(idmodulo)] = 0;      
      aI16_ControlHS[2][f_retornaIndiceModulo(idmodulo)] = resposta;
      aI16_ControlHS[3][f_retornaIndiceModulo(idmodulo)]++;
      if (aI16_ControlHS[3][f_retornaIndiceModulo(idmodulo)] > (vU8_tentativaConexoes*4)) {
        aI16_ControlHS[3][f_retornaIndiceModulo(idmodulo)] = vU8_tentativaConexoes-2;
      }
  }
  return resposta;
}
...
bool f_checkAcoesAssistentes() {
  ...
  if ((agora - check_acoes_ass_lasttime) > vU16_ass_MTBS) {
    for (uint8_t x=0; x<vU8_totPinos; x++) {
      if (aU8_AcaoRede1[2][x] && aU16_Pinos_Status[0][x] && !aU8_ControlMsgAss[0][x]) {
      ...
      } else if(aU8_AcaoRede1[2][x] && !aU16_Pinos_Status[0][x] && aU8_ControlMsgAss[0][x]){
      ...
      if (aU8_AcaoRede2[2][x] && aU16_Pinos_Status[0][x] && !aU8_ControlMsgAss[1][x]) {
      ...
      } else if(aU8_AcaoRede2[2][x] && !aU16_Pinos_Status[0][x] && aU8_ControlMsgAss[1][x]){
      ...
      if (aU8_AcaoRede3[2][x] && aU16_Pinos_Status[0][x] && !aU8_ControlMsgAss[2][x]) {
      ...
      } else if(aU8_AcaoRede3[2][x] && !aU16_Pinos_Status[0][x] && aU8_ControlMsgAss[2][x]){
    ...
    if (f_comunicaMQTT()) {
      for (uint8_t x=0; x<vU8_totPinos; x++) {
        if (aU8_AcaoRede1[3][x]) {
          ...
          f_publishDiscoveryMessage("mdi:shape-outline","sensor",f_traduzAcoesMqTT(aU16_Acao1[2][x]),String(aU16_Acao1[0][x])+"_"+String(aU16_Acao1[1][x]),aU16_Acao1[5][x]);
        } 
        if (aU8_AcaoRede2[3][x]) {
          ...
          f_publishDiscoveryMessage("mdi:shape-outline","sensor",f_traduzAcoesMqTT(aU16_Acao2[2][x]),String(aU16_Acao2[0][x])+"_"+String(aU16_Acao2[1][x]),aU16_Acao2[5][x]);
        } 
        if (aU8_AcaoRede3[3][x]) {
          ...
          f_publishDiscoveryMessage("mdi:shape-outline","sensor",f_traduzAcoesMqTT(aU16_Acao3[2][x]),String(aU16_Acao3[0][x])+"_"+String(aU16_Acao3[1][x]),aU16_Acao3[5][x]);
        } 
      }
    } else {
      f_reconnectMQTT();
...
bool f_checkAcoesMqTT() {
  ...
  if ((agora - check_acoes_mqtt_lasttime) > vU16_mqtt_MTBS) {
    if (f_comunicaMQTT()) {
      for (uint8_t x=0; x<vU8_totPinos; x++) {
        if (aU8_AcaoRede1[3][x]) {
          f_enviaMqTT(String(aU16_Acao1[0][x])+"_"+String(aU16_Acao1[1][x])+"/"+f_traduzAcoesMqTT(aU16_Acao1[2][x])+"/state",String(aU16_Pinos_Status[0][x]));
        }
        if (aU8_AcaoRede2[3][x]) {
          f_enviaMqTT(String(aU16_Acao2[0][x])+"_"+String(aU16_Acao2[1][x])+"/"+f_traduzAcoesMqTT(aU16_Acao2[2][x])+"/state",String(aU16_Pinos_Status[0][x]));
        }
        if (aU8_AcaoRede3[3][x]) {
          f_enviaMqTT(String(aU16_Acao3[0][x])+"_"+String(aU16_Acao3[1][x])+"/"+f_traduzAcoesMqTT(aU16_Acao3[2][x])+"/state",String(aU16_Pinos_Status[0][x]));
...
uint16_t f_carregaConfi(String ref, bool force) {
  ...
  if (ref == "ASSISTENTE" || ref == "TUDO") {
    ...
    vS_ass_Normal = CONFIG_FLASH.getString("ass_normal", "frase normalizacao");
  }
  if (ref == "MODULOS" || ref == "TUDO") {
    vB_exec_Modulos = CONFIG_FLASH.getBool("exe_mod", false);
    vU16_modulos_HandShake = CONFIG_FLASH.getULong64("time_mod", 65535);
    vU16_modulos_MTBS_Acoes = CONFIG_FLASH.getULong64("time_mod_ac", 65535);
...
bool f_checkAcoesTelegram() {
  ...
  if ((agora - check_acoes_telegram_lasttime) > vU16_bot_MTBS) {
    for (uint8_t x=0; x<vU8_totPinos; x++) {
      if (aU8_AcaoRede1[1][x] && aU16_Pinos_Status[0][x] && !aU8_ControlMsgTelegram[0][x]) {
      ...
      } else if(aU8_AcaoRede1[1][x] && !aU16_Pinos_Status[0][x] && aU8_ControlMsgTelegram[0][x]){
        resultado = f_enviaTelegram(vS_nomeDispositivo+"\n"+aS8_Pinos[0][x]+" - Normalização AÇÃO: "+f_traduzAcoesTelegram(aU16_Acao1[2][x])+"\n Origem: "+String(aU16_Acao1[0][x])+" Destino: "+aU16_Acao1[1][x]);
        aU8_ControlMsgTelegram[0][x] = 0;
      }
      if (aU8_AcaoRede2[1][x] && aU16_Pinos_Status[0][x] && !aU8_ControlMsgTelegram[1][x]) {
      ...
      } else if(aU8_AcaoRede2[1][x] && !aU16_Pinos_Status[0][x] && aU8_ControlMsgTelegram[1][x]){
        resultado = f_enviaTelegram(vS_nomeDispositivo+"\n"+aS8_Pinos[0][x]+" - Normalização AÇÃO 2: "+f_traduzAcoesTelegram(aU16_Acao2[2][x])+"\n Origem: "+String(aU16_Acao2[0][x])+" Destino: "+aU16_Acao2[1][x]);
        aU8_ControlMsgTelegram[1][x] = 0;
      }
      if (aU8_AcaoRede3[1][x] && aU16_Pinos_Status[0][x] && !aU8_ControlMsgTelegram[2][x]) {
      ...
      } else if(aU8_AcaoRede3[1][x] && !aU16_Pinos_Status[0][x] && aU8_ControlMsgTelegram[2][x]){
...
void f_salvaFlash() {
  ...
  CONFIG_FLASH.putULong64("time_mod", vU16_modulos_HandShake);
  CONFIG_FLASH.putULong64("time_mod_ac", vU16_modulos_MTBS_Acoes);
  CONFIG_FLASH.putULong64("ciclo_mod", vI_cicloHandshake);
  CONFIG_FLASH.putUChar("qnt_pinos", vU8_totPinos);


  Serial.print("Numero de elementos aU16_Pinos: ");
  Serial.println(sizeof(aU16_Pinos) / sizeof(uint8_t));
  Serial.print("Tamanho aU16_Pinos em bytes: ");
  Serial.println( sizeof(aU16_Pinos) );
  CONFIG_FLASH.putBytes( "paU16_Pinos", aU16_Pinos, sizeof(aU16_Pinos) );
  Serial.print("Tamanho paU16_Pinos em bytes: ");
  Serial.println( CONFIG_FLASH.getBytesLength("paU16_Pinos") );
  ...
  Serial.print("Numero de elementos aB_InterMod: ");
  Serial.println(sizeof(aB_InterMod) / sizeof(uint8_t)); //uint16_t
  Serial.print("Tamanho aB_InterMod em bytes: ");
  Serial.println( sizeof(aB_InterMod) );
  CONFIG_FLASH.putBytes( "paB_InterMod", aB_InterMod, sizeof(aB_InterMod) );
  Serial.print("Tamanho paB_InterMod em bytes: ");
  Serial.println( CONFIG_FLASH.getBytesLength("paB_InterMod") );
...
aU16_Pinos[4][x] = Envia ao Receptor 0=Nao / 1=Sim
...
*/
  ...
  uint16_t aU8BufferY[vI8_aU8_AcaoRede][vU8_totPinos] = {};
...
//========================================
void f_checkpisca(uint16_t pino) {
  if (aU16_Pinos_Status[0][f_retornaIndicePino(pino)]) {
    int x = aU16_Pinos_Status[0][f_retornaIndicePino(pino)];
    digitalWrite(pino,!digitalRead(pino));
    delay(100);
    digitalWrite(pino,!digitalRead(pino));
    delay(100);
    digitalWrite(pino,!digitalRead(pino));
    aU16_Pinos_Status[0][f_retornaIndicePino(pino)] = x;
  }
}

//========================================
void f_gravaPino(uint16_t tipo, uint16_t pino, uint16_t valor) {
  if (tipo == 1) {
...
//========================================
uint16_t f_lePino(uint16_t tipo, uint16_t invert, uint16_t pino) {
  uint16_t resultado = aU16_Pinos_Status[0][f_retornaIndicePino(pino)];
  if (tipo == 1) {
...
void f_iniciaPinos(bool force) {
  ...
  //vU8_bitsProc = CONFIG_FLASH.getUChar("bits_proc", 12);
  //vU8_ciclosAmostra = CONFIG_FLASH.getUChar("cicl_samp", 8);
...
bool f_configuraWEB() {
  ...
  if (WiFi.status() == 3) {
    ...
    //SERVIDOR_WEB.on("/analog", f_analogConf);
...
String f_DataHora() {
  String resultado;
  resultado += String(timeinfo.tm_mday);
  resultado += "/";
  resultado += aS_traduzMes[0][timeinfo.tm_mon];
  resultado += " - ";
  resultado += String(timeinfo.tm_hour);
  resultado += ":";
  resultado += String(timeinfo.tm_min);
  resultado += ":";
  resultado += String(timeinfo.tm_sec);
  return resultado;
}
...
uint8_t  f_configuraWIFI() {
  ...
  if (WiFi.status() == 3) {
    ...
    f_config_NTP();
  } else if (vB_modoAP && WiFi.status() != 3 && !vB_emExecucaoWifi) {
