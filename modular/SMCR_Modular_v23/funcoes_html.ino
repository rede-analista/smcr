...
void f_handle_ConfiguraGeral() {
  ...
  html += "<input type='text' style='background-color: LightGrey' name='TOTPIN' id='id_totpin' value='"+String(vU8_totPinos)+"' readonly required> 2 a 52</label>";
  html += "<br><br>";
...
//  html += "<br>";
//  html += "<label for='id_portarec'>Porta do Receptor(geral): ";
//  html += "<input type='text' name='PORTAREC' id='id_ipporta' maxlength='14' size='14' value='"+String(vU16_portaWebModulos)+"'> 20 a 65535 </label> <a href='https://pt.wikipedia.org/wiki/Lista_de_portas_dos_protocolos_TCP_e_UDP' target='_blank'>Ajuda</a>";  
  html += "<br>";
  html += "<br>";  
...
void f_handle_ConfiguraWifi() {
  ...
  html += "<br><br><a href=\"/configurag\">Config Geral</a>";
  html += "<br><a href=\"/\">Página Inicial</a>\n";
  html += "</body>";
  html += "</html>";
  SERVIDOR_WEB.send(200, "text/html", html);
...
*/
  ...
  html += "Pino HandShake dos Módulos<br>";
...
void f_listaPreferencesMEM() {
  ...
  for (uint8_t x=0; x<vI8_aU8_ControlMsgHist; x++){
    ...
    html += " = {";
    for (uint8_t y=0; y<vU8_totPinos; y++) {
      html += aU8_ControlMsgModHist[x][y];
      if (y < vU8_totPinos-1) {
        html += ",";
      }
    }
