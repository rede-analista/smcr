//========================================
void f_handle_ConfiguraAcoes1pg1() {
  String html;
  ...
  html += "<title>"+vS_nomeDispositivo+" Casdastro de Ações Pg1</title>";
  html += "</head>";
  html += "<body id='body'>";
  html += "<div id='telaConfigAcoes'> ";
  html += "<div id='bloqueiaTela'></div>";
  html += "<h1>Cadastro de Ações</h1>";
  html += "<div id='mensagens'><p>Insira as informações de configuração das 1ª Ação(pinos 1 a "+String((vU8_totPinos/2))+"). e em seguida clique em \"Aplicar\".</p></div>";
  ...
  html += "<form action='/acoes1pg1' method='POST' style='margin:5px'>";
  html += "<input type='submit' name='SUBMIT_SALVAR' value='Aplicar 1ª Ação Pg1(sem salvar)' id='id_salvar'>";
  html += "<br><div style='background-color:Tomato'>!! APLIQUE AS ALTERAÇÕES ANTES DE MUDAR DE PÁGINA !! </div><br>";
  ...
  while (linha >= 0) {
    ...
    for (uint8_t xM = 0; xM<vI8_aU16_Acao;xM++) {
      ...
      for (x=colINICIO; x<colFIM; x++){
        if (xM == 0) {
          html += "<td><center><input type='text' style='background-color: LightGrey' name='"+aS8_AcoesMenu[0][xM]+String(x)+"' maxlength='14' size='14' value='"+String(aU16_Pinos[xM][x])+"' readonly required></center></td>";
        } else {
          html += "<td><center><input type='text' name='"+aS8_AcoesMenu[0][xM]+String(x)+"' maxlength='14' size='14' value='"+String(aU16_Acao1[xM][x])+"'></center></td>";
        }
      }
      html += "</tr>";
    }

    for (uint8_t xM = 0; xM<vI8_aU8_AcaoRede;xM++) {
      html += "<tr>";
      html += "<td style='background-color: LightGrey'><center>"+aS8_AcoesRedeMenu[0][xM]+"</center></td>";
      for (x=colINICIO; x<colFIM; x++){
        if (xM == 0) {
          html += "<td><center><input type='text' name='"+aS8_AcoesRedeMenu[0][xM]+String(x)+"' maxlength='14' size='14' value='"+String(aU8_AcaoRede1[xM][x])+"'></center></td>";
        } else {
          html += "<td><center><input type='checkbox' name='"+aS8_AcoesRedeMenu[0][xM]+String(x)+"' value='1' "+String((aU8_AcaoRede1[xM][x] ? "checked" : ""))+"></center></td>";
        }
      }
      html += "</tr>";
    }
  ...
  html += "</table>";
  html += "<input type='submit' name='SUBMIT_SALVAR' value='Aplicar 1ª Ação Pg1(sem salvar)' id='id_salvar'><br>";
  html += "</form>";
  html += "</div>";
  Serial.println("Aguardando informacoes... f_handle_ConfiguraAcoes1pg1");
  if (SERVIDOR_WEB.arg("SUBMIT_SALVAR").length() > 0) {
    ...
    for (x=0; x<colFIM; x++){
      for (uint8_t xM = 0; xM<vI8_aU16_Acao;xM++) {
        aU16_Acao1[xM][x] = SERVIDOR_WEB.arg((aS8_AcoesMenu[0][xM]+String(x))).toInt();
      }
      for (uint8_t xM = 0; xM<vI8_aU8_AcaoRede;xM++) {
          aU8_AcaoRede1[xM][x] = SERVIDOR_WEB.arg((aS8_AcoesRedeMenu[0][xM]+String(x))).toInt();
          if(aU8_AcaoRede1[3][x]) {
            addass = true;
            CLIENT_ASS_ALEXA.addDevice(f_traduzAcoesAss(aU16_Acao1[2][x])+" "+aS8_Pinos[0][x], f_callbackNotificaAlexa);
          }
      }
    }
    if(addass) {
