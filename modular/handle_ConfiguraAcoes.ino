...
void f_handle_ConfiguraAcoes() {
  ...
  while (linha > 0) { 
    for (uint8_t xM = 0; xM<vI8_aU16_Acao;xM++) {
      ...
      for (x=colINICIO; x<colFIM; x++){
        if (xM == 0) {
          ...
          html += "<td><center><input type='text' name='"+aS8_AcoesMenu[0][xM]+String(x)+"' value='"+String(aU16_Acao1[xM][x])+"'></center></td>";
        }
      }
      html += "    </tr>";
    }
    for (uint8_t xM = 0; xM<vI8_aU16_AcaoRede;xM++) {
      ...
      for (x=colINICIO; x<colFIM; x++){
        html += "<td><center><input type='text' name='"+aS8_AcoesRedeMenu[0][xM]+String(x)+"' value='"+String(aU16_AcaoRede1[xM][x])+"'></center></td>";
      }
      html += "    </tr>";
    }
    for (uint8_t xM = 0; xM<vI8_aS8_Acao;xM++) {
      ...
      for (x=colINICIO; x<colFIM; x++){
        html += "<td><center><input type='text' name='"+aS8_AcoesStringMenu[0][xM]+String(x)+"' value='"+String(aS8_Acao1[xM][x])+"'></center></td>";
      }
      html += "    </tr>";
    }    
    html += "    <tr>";
