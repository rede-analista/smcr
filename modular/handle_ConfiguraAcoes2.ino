...
void f_handle_ConfiguraAcoes2() {
  ...
  html += "<h1>Insira as informações das Ações Pg 2</h1>";
  html += "<div id='mensagens'><p>Insira novas informações de configuração das Ações Pg 2. Em seguida, clique em Aplicar para enviar os dados para o ["+vS_nomeDispositivo+"]: </p></div>";
  ...
  while (linha > 0) {
    ...
    html += "        <td style='background-color: LightGrey'><center>Pino Destino</center></td>";
                    for (x=colINICIO; x<colFIM; x++){
                      html += "        <td><center><input type='text' name='PINDES"+String(x)+"' value='"+String(aU16_Acao2[1][x])+"' required></center></td>";
                    }
    html += "    </tr>";
    html += "    <tr>";
    html += "        <td style='background-color: LightGrey'><center>Ação</center></td>";
                    for (x=colINICIO; x<colFIM; x++){
                      html += "        <td><center><input type='text' name='ACAO"+String(x)+"' value='"+String(aU16_Acao2[2][x])+"' required></center></td>";
                    }
    html += "    </tr>";
    html += "    <tr>";
    html += "        <td style='background-color: LightGrey'><center>Nível Acion.</center></td>";
                    for (x=colINICIO; x<colFIM; x++){
                      html += "        <td><center><input type='text' name='NIVAC"+String(x)+"' value='"+String(aU16_Acao2[9][x])+"' required></center></td>";
                    }
    html += "    </tr>";    
    html += "    <tr>";                  
    html += "        <td style='background-color: LightGrey'><center>Tempo ON</center></td>";
                    for (x=colINICIO; x<colFIM; x++){
                      html += "        <td><center><input type='text' name='TON"+String(x)+"' value='"+String(aU16_Acao2[3][x])+"' required></center></td>";
                    }
    html += "    </tr>";
    html += "    <tr>";
    html += "        <td style='background-color: LightGrey'><center>Tempo OFF</center></td>";
                    for (x=colINICIO; x<colFIM; x++){
                      html += "        <td><center><input type='text' name='TOFF"+String(x)+"' value='"+String(aU16_Acao2[4][x])+"' required></center></td>";
                    }
    html += "    </tr>";
    html += "    <tr>";
    html += "        <td style='background-color: LightGrey'><center>Envia Receptor</center></td>";
                    for (x=colINICIO; x<colFIM; x++){
                      html += "        <td><center><input type='text' name='EVREC"+String(x)+"' value='"+String(aU16_Acao2[5][x])+"' required></center></td>";
                    }
    html += "    </tr>";
    html += "    <tr>";
    html += "        <td style='background-color: LightGrey'><center>Envia Telegram</center></td>";
                    for (x=colINICIO; x<colFIM; x++){
                      html += "        <td><center><input type='text' name='EVTEL"+String(x)+"' value='"+String(aU16_Acao2[6][x])+"' required></center></td>";
                    }
    html += "    </tr>";
    html += "    <tr>";
    html += "        <td style='background-color: LightGrey'><center>Envia MqTT</center></td>";
                    for (x=colINICIO; x<colFIM; x++){
                      html += "        <td><center><input type='text' name='EVMQT"+String(x)+"' value='"+String(aU16_Acao2[7][x])+"' required></center></td>";
                    }
    html += "    </tr>";
    html += "    <tr>";
    html += "        <td style='background-color: LightGrey'><center>Classe MqTT </center></td>";
                    for (x=colINICIO; x<colFIM; x++){
                      html += "        <td><center><input type='text' name='CLSMQT"+String(x)+"' value='"+aS8_Acao2[0][x]+"' required></center></td>";
                    }
    html += "    </tr>";  
    html += "    <tr>";
    html += "        <td style='background-color: LightGrey'><center>Icone MqTT </center></td>";
                    for (x=colINICIO; x<colFIM; x++){
                      html += "        <td><center><input type='text' name='ICOMQT"+String(x)+"' value='"+aS8_Acao2[1][x]+"'></center></td>";
                    }
    html += "    </tr>";          
    html += "    <tr>";
    html += "        <td style='background-color: LightGrey'><center>Not. Assist.</center></td>";
                    for (x=colINICIO; x<colFIM; x++){
                      html += "        <td><center><input type='text' name='NOTASS"+String(x)+"' value='"+String(aU16_Acao2[8][x])+"' required></center></td>";
                    }
    html += "    </tr>";
    html += "    <tr>";
  ...
  html += "</table>";
  html += "<br>";
  html += "<input type='submit' name='SUBMIT_SALVAR' value='Aplicar Pg:2 (sem salvar)' id='id_salvar'>";
  html += "</form>";
  html += "</div>";
  html += "</div>";
  Serial.println("Aguardando informacoes... f_handle_ConfiguraAcoes2()");
  if (SERVIDOR_WEB.arg("SUBMIT_SALVAR").length() > 0) {
    Serial.print("Atualizando Informacoes Acoes: ");
    for (x=0; x<vU8_totPinos; x++){
      aU16_Acao2[0][x] = SERVIDOR_WEB.arg(("PINORI"+String(x))).toInt();
      aU16_Acao2[1][x] = SERVIDOR_WEB.arg(("PINDES"+String(x))).toInt();
      aU16_Acao2[2][x] = SERVIDOR_WEB.arg(("ACAO"+String(x))).toInt();
      aU16_Acao2[3][x] = SERVIDOR_WEB.arg(("TON"+String(x))).toInt();
      aU16_Acao2[4][x] = SERVIDOR_WEB.arg(("TOFF"+String(x))).toInt();
      aU16_Acao2[5][x] = SERVIDOR_WEB.arg(("EVREC"+String(x))).toInt();
      aU16_Acao2[6][x] = SERVIDOR_WEB.arg(("EVTEL"+String(x))).toInt();
      aU16_Acao2[7][x] = SERVIDOR_WEB.arg(("EVMQT"+String(x))).toInt();
      aU16_Acao2[8][x] = SERVIDOR_WEB.arg(("NOTASS"+String(x))).toInt();
      aU16_Acao2[9][x] = SERVIDOR_WEB.arg(("NIVAC"+String(x))).toInt();
      aS8_Acao2[0][x] = SERVIDOR_WEB.arg(("CLSMQT"+String(x)));
      aS8_Acao2[1][x] = SERVIDOR_WEB.arg(("ICOMQT"+String(x)));      
    }
    Serial.println("OK");
