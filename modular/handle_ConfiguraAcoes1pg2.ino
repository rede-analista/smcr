...
void f_handle_ConfiguraAcoes1pg2() {
  ...
  html += "<form action='/acoes1pg2' method='POST' style='margin:5px'>";
    html += "<input type='submit' name='SUBMIT_SALVAR' value='Aplicar Pg2(sem salvar)' id='id_salvar'><br>";
  ...
  while (linha >= 0) {
    ...
    for (uint8_t xM = 0; xM<vI8_aS8_Acao;xM++) {
      ...
      for (x=colINICIO; x<colFIM; x++){
        html += "<td><center><input type='text' name='"+aS8_AcoesStringMenu[0][xM]+String(x)+"' maxlength='14' size='14' value='"+aS8_Acao1[xM][x]+"'></center></td>";
      }
      html += "</tr>";
    }    
  ...
  html += "</table>";
  html += "<input type='submit' name='SUBMIT_SALVAR' value='Aplicar Pg2(sem salvar)' id='id_salvar'>";
  html += "</form>";
  html += "</div>";
